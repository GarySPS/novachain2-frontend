...
export default function AiMiningPage() {
  ...
  const handleAllocate = async (e) => {
    ...
    if (usdtEquivalent < 50) {
      alert("Minimum allocation is $50 USDT equivalent.");
      setIsProcessing(false);
      return;
    }
  ...
  return (
    <div className="relative min-h-screen w-full overflow-y-auto pb-24 text-white">
      ...
      <div className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-white/5 bg-[#0b1020]/80 px-4 backdrop-blur-md">
        ...
        <h1 className="text-lg font-black tracking-wide text-emerald-400">ETH AI Mining</h1>
      ...
      <div className="mx-auto max-w-lg space-y-4 p-4">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-[#141a2b] to-[#0b1020] p-6 shadow-[0_10px_40px_rgba(16,185,129,0.1)]">
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-emerald-400">
              ...
              Nodes Online & Hashing
            </div>
          </div>

          <div className="mt-8 grid grid-cols-2 gap-4 border-t border-white/5 pt-6">
            <div>
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Current Yield</div>
              <div className="mt-1 text-2xl font-black text-white">{currentYield}% <span className="text-sm text-slate-500">Weekly</span></div>
            </div>
            <div className="text-right">
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Est. Payout</div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2 rounded-2xl border border-white/5 bg-[#141a2b] p-4">
            <p className="text-xs leading-relaxed text-slate-400">
              <strong className="text-emerald-400">AI Engine Active.</strong> Dynamically allocating capital across top Ethereum Layer-2 nodes. ETH block rewards are automatically liquidated and secured in USDT to protect against market volatility. 
            </p>
          </div>

          {/* Uptime Block */}
          <div className="rounded-2xl border border-white/5 bg-[#141a2b] p-4">
            <div className="mb-2 text-[10px] font-black uppercase tracking-widest text-slate-500">Node Uptime</div>
          ...
          <div className="rounded-2xl border border-white/5 bg-[#141a2b] p-4 text-right">
            <div className="mb-2 text-[10px] font-black uppercase tracking-widest text-slate-500">Next Payout</div>
            <div className="text-lg font-black text-white tabular-nums">
              {timeLeft.d}d {String(timeLeft.h).padStart(2, '0')}:{String(timeLeft.m).padStart(2, '0')}
            </div>
            <div className="mt-1 text-[10px] font-bold text-rose-400">7-Day Lock Active</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          ...
          >
            <Icon name="zap" className="h-4 w-4" /> Deploy Capital
          </button>
          ...
          >
            <Icon name="log-out" className="h-4 w-4" /> Withdraw
          </button>
        </div>

        <div className="mt-6 rounded-2xl border border-white/5 bg-[#141a2b] p-4">
          <h3 className="mb-4 text-xs font-black uppercase tracking-widest text-slate-400">Yield Tiers</h3>
              ...
              return (
                ...
                >
                  ...
                  <div className={`font-black ${isActive ? "text-emerald-400" : "text-slate-300"}`}>
                    {tier.yield}% <span className="text-[10px] font-normal text-slate-500">/wk</span>
                  </div>
                </div>
              );
            })}
      ...
      <Modal visible={modal.open} onClose={() => setModal({ open: false, type: "" })} classWrap="bg-[#0f1424] border border-[#1a2343]">
        ...
        {modal.type === "deposit" && (
          <div className="p-2">
            <h2 className="mb-2 text-center text-xl font-black text-white">Allocate Capital</h2>
            <p className="mb-6 text-center text-xs text-slate-400">Deposit any asset. It will be auto-converted to USDT to secure your mining contract.</p>

            <form onSubmit={handleAllocate} className="space-y-4">
              <div>
                <label className="mb-1 block text-xs font-bold text-slate-400">Select Asset</label>
                <select
                  value={selectedAsset}
                ...
                >
                  {walletBalances.map(b => (
                    <option key={b.symbol} value={b.symbol}>{b.symbol} (Avail: {Number(b.balance).toFixed(4)})</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                {depositAmount && selectedAsset !== "USDT" && prices[selectedAsset] && (
                  <div className="mt-2 text-right text-xs font-bold text-emerald-400">
                    Auto-Converts to ≈ {fmtUSD(parseFloat(depositAmount) * prices[selectedAsset])}
                  </div>
                )}
              </div>
              ...
              >
                {isProcessing ? "Allocating Node Power..." : "Confirm Allocation"}
              </button>
            </form>
          </div>
        )}
        ...
        {modal.type === "withdraw" && (
          <div className="p-2">
            ...
            <h2 className="mb-2 text-center text-xl font-black text-white">Early Withdrawal</h2>
            
            <div className="mb-6 rounded-xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm text-rose-200">
              <p className="mb-3 text-center font-bold text-rose-400">Warning: Breaking the 7-day cycle</p>
              <ul className="list-inside list-disc space-y-2 text-xs text-rose-300">
                <li>Your primary capital will be instantly returned to your Spot Wallet.</li>
                <li><strong className="text-white">All accumulated mining profits for this current cycle will be forfeited.</strong></li>
                <li>When you redeposit, the 7-day timer will restart entirely from Day 1.</li>
              </ul>
            </div>
            ...
            <form onSubmit={handleWithdraw} className="space-y-4">
              <div>
                <div className="mb-1 flex justify-between text-xs font-bold">
                  <span className="text-slate-400">Withdraw Amount (USDT)</span>
                  <span className="text-emerald-400">Available: {fmtUSD(miningCapital)}</span>
                </div>
              ...
              >
                {isProcessing ? "Processing..." : "Acknowledge & Withdraw"}
              </button>
            </form>
          </div>
        )}
